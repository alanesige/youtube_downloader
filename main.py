...
"""
def songs_in_txt():
        with open('hop.txt', 'r', newline='', encoding="cp437", errors='ignore') as f:
...
def delete_downloaded_song():
        with open("hop.txt", 'r+', encoding="cp437", errors='ignore') as fp:
            lines = fp.readlines()
            fp.seek(0)
            fp.truncate()
...
def put_it_there(songer):
    ...
    with open("undownloaded.txt", "a", encoding="cp437", errors='ignore') as f:
        f.write("\n")
        f.write(songer)         
